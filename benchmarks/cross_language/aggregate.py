...
def main() -> None:
    ...
    lines.append("")
    lines.append(
        "- CI에서는 각 구현을 같은 러너에서 순차 실행합니다. 로컬 실행 환경은 결과 JSON을 확인하세요."
    )
    lines.append(
        "- Python은 5회, JavaScript는 20회 워밍업한 뒤 샘플 시간이 약 30 ms 이상이 되도록 반복 횟수를 자동 보정했습니다."
    )
    lines.append("- 각 항목은 9개 샘플의 중앙값을 사용합니다.")
    lines.append(
        "- 문자열 연산은 문자 또는 자모 1개당 시간, 배치 연산은 단어·숫자·쿼리 1개당 시간으로 정규화했습니다."
    )
    lines.append(
        "- 전체 측정 입력의 출력을 SHA-256으로 비교합니다. 출력 또는 단위가 다르면 순위와 배수를 표시하지 않습니다."
    )
    lines.append(
        "- 낮을수록 빠릅니다. `hangulpy 대비`가 0.50x라면 hangulpy 시간의 절반, 즉 약 2배 빠른 것입니다."
    )
    lines.append("")
    ...
    for feature in feature_order:
        ...
        comparable = (
            all(item.get("output_sha256") for item in candidates)
            and len({item["output_sha256"] for item in candidates}) == 1
            and len({item["unit"] for item in candidates}) == 1
        )
        if comparable:
        ...
        if not comparable:
            lines.append(
                "출력 또는 단위가 일치하지 않아 순위에서 제외합니다. 표기 정책·검색 의미 차이를 확인하세요."
            )
            lines.append("")
        ...
        for rank, item in enumerate(candidates, start=1):
            ...
            if baseline_ns is None or not comparable:
                ratio = "-"
            else:
            ...
            lines.append(
                "| "
                f"{rank if comparable else '-'} | {item['library']} | {item['language']} | {item['version']} | "
    ...
    lines.append("")
    lines.append(
        "- 언어 런타임 자체의 비용까지 포함한 실제 라이브러리 호출 비용 비교입니다. Python과 V8의 실행 모델 차이는 결과의 일부입니다."
    )
    lines.append(
        "- JIT 기반 JavaScript는 워밍업 후 정상 상태 성능을 측정합니다. 짧은 CLI의 최초 호출 지연은 별도 지표가 아닙니다."
    )
    lines.append(
        "- 기능이 비슷해도 예외 처리, Unicode 범위, 숫자·영문 약어 지원 범위가 다른 경우가 있으므로 속도만으로 API 품질을 판단하면 안 됩니다."
    )
    lines.append(
        "- 현재 1차 러닝은 직접 대응 가능한 공개 API 중심입니다. 퍼지 거리, 한자 분할, 고급 로마자 모드처럼 동등한 경쟁 API가 없는 기능은 순위에서 제외했습니다."
    )
    lines.append("")
