...
문자열 전체를 로마자로 변환하는 편의 함수입니다. 기본 `revised` 모드는
[국립국어원 국어의 로마자 표기법](https://korean.go.kr/kornorms/regltn/regltnView.do?regltn_code=0004)의
주요 규칙을 구현합니다. 어휘 사전이나 형태소 분석기를 포함하지 않으므로 모든
단어의 규정 표기를 보장하지는 않습니다. ㄴ 첨가는 지원하는 어휘와 조사 결합에
적용하며, `금요일`처럼 첨가 대상이 아닌 단어에는 임의로 ㄴ을 넣지 않습니다.
